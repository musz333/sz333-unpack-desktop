...
/* ------------------------------------------------------------------ *
 * 源文件处理：移到回收站（Windows PowerShell 调用，失败则退回直接删除）
 * ------------------------------------------------------------------ */
export async function moveToRecycleBin(targets: string[]): Promise<{ moved: number; failed: string[] }> {
  if (!targets.length) return { moved: 0, failed: [] };
  const failed: string[] = [];
  let moved = 0;

  // 优先使用 Shell.Application 的 Recycle（Windows 内置，无需额外依赖）
  ...
  const ps = [
    ...
    '      $isDir = (Get-Item -LiteralPath $i).PSIsContainer',
    '      if ($isDir) { [Microsoft.VisualBasic.FileIO.FileSystem]::DeleteDirectory($i, "OnlyErrorDialogs", "SendToRecycleBin") }',
    '      else { [Microsoft.VisualBasic.FileIO.FileSystem]::DeleteFile($i, "OnlyErrorDialogs", "SendToRecycleBin") }',
    '      $ok++',
    '    }',
    '  } catch { }',
    '}',
  ...
  try {
    ...
    moved = parseInt(stdout.trim().split(/\r?\n/).pop() || '0', 10) || 0;
