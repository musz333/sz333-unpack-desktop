...
| 文件 | 说明 |
| --- | --- |
| `sz333 解压工具-1.0.4-x64-setup.exe` | NSIS 安装包（可选安装目录、创建快捷方式） |
| `sz333 解压工具-1.0.4-便携版.exe` | 免安装便携版，双击即用 |
...
| **APK 过滤（可选）** | 默认关闭。开启后：① 解压产物里的 `.apk` 一律剔除；② 若**单个 apk** 占整包解出体积达到阈值（默认 **70%**，可调），判定这个包只是个 apk 外壳，连整份产物一起删掉。删的是**解压产物**，不碰源压缩包（源包去留由「源文件处理」单独决定） |
